...
static ssize_t store_scp_ctrl(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (kstrtoul(buf, 10, &cmd)) {
		printk("[SCP_CTRL]: Invalid values\n");
		return -EINVAL;
	}

	printk("SCP_CTRL: Command=%d", cmd);
	switch (cmd) {
	case 1:
	...
	case 5:
		{
			...
			if (md32_ipi_send(IPI_TOUCH, &ipi_pkt, sizeof(ipi_pkt), 0) < 0)
				printk("[TOUCH] IPI cmd failed (%d)\n", ipi_pkt.cmd);

			break;
		}
	default:
	    printk("[SCP_CTRL] Unknown command");
	    break;
	}
...
static int touch_event_handler(void *unused)
{
	...
	printk("touch_event_handler exit\n");
...
static s8 ftp_enter_doze(struct i2c_client *client)
{
	...
	if (ret < 0) {
		/* printk("Failed to enter Doze %d", retry); */
	...
	for (i = 0; i < 10; i++) {
		...
		if (gestrue_data == 0x01) {
			...
			/* printk("FTP has been working in doze mode!"); */
...
static void tpd_suspend(struct device *h)
{
	...
	/* printk("[tpd_scp_doze]:init=%d en=%d", scp_init_flag, tpd_scp_doze_en); */
	...
	if (scp_init_flag == 0) {
		...
		printk("[TOUCH]SEND CUST command :%d ", IPI_COMMAND_AS_CUST_PARAMETER);

		ret = md32_ipi_send(IPI_TOUCH, &ipi_pkt, sizeof(ipi_pkt), 0);
		if (ret < 0)
			printk(" IPI cmd failed (%d)\n", ipi_pkt.cmd);
	...
	if (tpd_scp_doze_en) {
		printk("[TOUCH]SEND ENABLE GES command :%d ", IPI_COMMAND_AS_ENABLE_GESTURE);
		ret = ftp_enter_doze(i2c_client);
		if (ret < 0) {
			printk("FTP Enter Doze mode failed\n");
	  } else {
			int retry = 5;
	    {
				/* check doze mode */
				fts_read_reg(i2c_client, FT_GESTRUE_MODE_SWITCH_REG, &gestrue_data);
				printk("========================>0x%x", gestrue_data);
	    }
	    ...
	    if (retry <= 0)
				printk("############# md32_ipi_send failed retry=%d", retry);

			/*while(release_md32_semaphore(SEMAPHORE_TOUCH) <= 0) {
				//printk("GTP release md32 sem failed\n");
